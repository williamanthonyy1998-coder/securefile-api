...
r.post('/send', auth, activeSubscription, upload.single('file'), async (req:AuthedRequest,res,next)=>{
  try{
    ...
    try{
      ...
      await notify(req.user!.id,'Fax queued',`Your fax to ${to} has been queued for delivery.`,companyId, undefined, true);
      res.status(201).json(updated);
    }catch(e:any){
      await db.faxJob.update({where:{id:job.id},data:{status:'FAILED',errorMessage:e.message||'Fax provider error'}});
      await notify(req.user!.id,'Fax failed',e.message||`Unable to send fax to ${to}.`,companyId, undefined, true);
      throw e;
    }
  }catch(e){next(e)}
});
