...
r.post('/number/provision', auth, activeSubscription, async (req:AuthedRequest,res,next)=>{
  try{
    ...
    await notify(req.user!.id,'Your SecureFile fax number is ready',`Your personal fax number is ${phoneNumber}.`,companyId);
...
r.post('/send', auth, activeSubscription, upload.single('file'), async (req:AuthedRequest,res,next)=>{
  try{
    ...
    if(req.body.fileId){
      ...
      const object=await getObject(f.storageKey); if(!object)return res.status(404).json({error:'Stored file is missing.'});
      buffer=object;filename=f.name;fileId=f.id;
    } else if(req.file){
      buffer=req.file.buffer;filename=req.file.originalname||filename;
    } else return res.status(400).json({error:'Choose a SecureFile document or upload a document to fax.'});
    ...
    try{
      const result:any=await sendPhaxioFax({to,buffer,filename,headerText:String(req.body.headerText||'').slice(0,50)||undefined,callerId:line.phoneNumber,callbackUrl:callbackUrl(),tag:{securefile_job_id:job.id,user_id:req.user!.id}});
      const providerRef=String(result.id||result.fax_id||'');
      const updated=await db.faxJob.update({where:{id:job.id},data:{status:'QUEUED',providerRef:providerRef||null}});
      await notify(req.user!.id,'Fax queued',`Your fax to ${to} has been queued for delivery.`,companyId);
      res.status(201).json(updated);
    }catch(e:any){
      await db.faxJob.update({where:{id:job.id},data:{status:'FAILED',errorMessage:e.message||'Fax provider error'}});
      await notify(req.user!.id,'Fax failed',e.message||`Unable to send fax to ${to}.`,companyId);
      throw e;
    }
  }catch(e){next(e)}
});
