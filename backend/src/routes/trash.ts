...
const restoreHandler=async(req:AuthedRequest,res:any,next:any)=>{try{
 ...
 if(type==='FILE'){
   ...
   const parent=f.folderId?await db.folder.findFirst({where:{id:f.folderId,companyId},select:{deletedAt:true}}):null;await db.file.update({where:{id},data:{deletedAt:null,folderId:parent?.deletedAt?null:f.folderId}}); await audit(companyId,req.user!.id,'RESTORE','FILE',id); await notifyCompanyAdmins(companyId,'File restored',`${f.name} was restored from Trash by ${req.user!.email||'a user'}.`,'FILE_UPDATED',{excludeUserId:req.user!.id,entityId:id}); return res.json({ok:true});
 }
 if(type==='FOLDER'){
   ...
   await audit(companyId,req.user!.id,'RESTORE','FOLDER',id); await notifyCompanyAdmins(companyId,'Folder restored',`${f.name} was restored from Trash by ${req.user!.email||'a user'}.`,'FILE_UPDATED',{excludeUserId:req.user!.id,entityId:id}); return res.json({ok:true});
 }
...
r.delete('/:type/:id',auth,activeSubscription,async(req:AuthedRequest,res,next)=>{try{
 ...
 if(type==='FILE'){
   ...
   await deleteObject(f.storageKey).catch(()=>{}); await db.file.delete({where:{id}}); await db.company.update({where:{id:companyId},data:{storageUsedBytes:{decrement:f.sizeBytes}}}); await notifyCompanyAdmins(companyId,'File permanently deleted',`${f.name} was permanently deleted by ${req.user!.email||'a user'}.`,'FILE_DELETED',{excludeUserId:req.user!.id,entityId:id}); return res.status(204).end();
 }
 if(type==='FOLDER'){
   const f=await db.folder.findFirst({where:{id,companyId,deletedAt:{not:null}}}); if(!f)return res.status(404).json({error:'Deleted folder not found'});
   ...
   const total=files.reduce((n,x)=>n+x.sizeBytes,0n); await db.company.update({where:{id:companyId},data:{storageUsedBytes:{decrement:total}}}); await notifyCompanyAdmins(companyId,'Folder permanently deleted',`${f.name} was permanently deleted by ${req.user!.email||'a user'}.`,'FILE_DELETED',{excludeUserId:req.user!.id,entityId:id}); return res.status(204).end();
 }
 res.status(400).json({error:'Invalid trash item type'});
}catch(e){next(e)}});
export default r;
