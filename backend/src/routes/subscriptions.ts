...
r.post('/cancel',auth,role('COMPANY_ADMIN'),async(req:AuthedRequest,res,next)=>{try{
  ...
  for(const u of users)await notify(u.id,'Subscription canceled','Your SecureFile workspace is now suspended. Your data is preserved. Renew from Settings to restore full access.',companyId,undefined,true);
...
r.post('/reactivate',auth,role('COMPANY_ADMIN'),async(req:AuthedRequest,res,next)=>{try{
  ...
  const {notify}=await import('../services/notify'); const users=await db.user.findMany({where:{companyId,status:{not:'SUSPENDED'}},select:{id:true}});
  for(const u of users)await notify(u.id,'Subscription reactivated','Your current paid SecureFile period will continue normally.',companyId,undefined,true);
    ...
    if(event.type==='checkout.session.completed'){
      ...
      if(companyId&&subscriptionId){
        ...
        if(current){
          ...
          for(const u of usersToNotify) await notify(u.id,'Payment successful',`Your SecureFile access is active for ${months} month${months===1?'':'s'} after your upfront payment of $${(totalPriceCents/100).toFixed(2)}.`,companyId,undefined,true);
        }
      }
      ...
      if(stripeSubId){
        ...
        if(current){
          ...
          for(const u of users) await notify(u.id,'Subscription renewed','Your monthly SecureFile payment was received successfully.',current.companyId,undefined,true);
        }
      }
      ...
      if(stripeSubId){
        ...
        if(current){
          ...
          const {notify}=await import('../services/notify');
          const users=await db.user.findMany({where:{companyId:current.companyId,status:{not:'SUSPENDED'}},select:{id:true}});
          for(const u of users) await notify(u.id,'Payment failed','Your SecureFile subscription payment failed. Please update your payment method to avoid service interruption.',current.companyId,undefined,true);
        }
      }
      ...
      if(current){
        ...
        const {notify}=await import('../services/notify');
        const users=await db.user.findMany({where:{companyId:current.companyId,status:{not:'SUSPENDED'}},select:{id:true}});
        for(const u of users) await notify(u.id,'Subscription canceled','Your SecureFile subscription has been canceled.',current.companyId,undefined,true);
      }
    }
