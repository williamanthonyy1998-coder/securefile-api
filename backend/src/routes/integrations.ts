...
r.post('/fax/inbound', upload.single('file'), async (req, res) => {
  ...
  await notify(userId, 'New fax received', `A new fax was received on your personal fax number.`, companyId);
  ...
  try {
    ...
    if (direction === 'received' || req.file) {
      ...
      try {
        ...
        await notify(line.userId, 'New fax received', `${name} was received on your personal fax number.`, line.companyId);
    ...
    await notify(job.userId, failed ? 'Fax delivery failed' : 'Fax delivered', failed ? `Your fax to ${job.recipientNumber || 'the recipient'} could not be delivered.` : `Your fax to ${job.recipientNumber || 'the recipient'} was delivered successfully.`, job.companyId);
