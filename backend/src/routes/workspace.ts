...
r.post('/requests',auth,activeSubscription,async(req:AuthedRequest,res,next)=>{try{
 ...
 const requester=await db.user.findUnique({where:{id:req.user!.id},select:{uniqueName:true,email:true}});const message=`${requester?.uniqueName||requester?.email||'A user'} is requesting ${requestedType.toLowerCase()} “${requestedName}”.${note?' Reason: '+note:''}`;await notify(targetUserId,'New access request',message,companyId,'ACCESS_REQUESTED',true,{entityId:result.id});await sendUserEmail(target.email,'SecureFile access request',`<p>${message}</p><p>Open SecureFile → Approvals to review and fulfill this request.</p>`).catch(()=>{});res.status(201).json(result);
}catch(e){next(e)}});
