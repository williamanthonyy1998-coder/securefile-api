...
import {db} from '../db';
export type AuthedUser={id:string,role:string,companyId:string|null,email:string};
export type AuthedRequest=Request & {user?:AuthedUser};
export async function auth(req:AuthedRequest,res:Response,next:NextFunction){const h=req.headers.authorization;if(!h?.startsWith('Bearer '))return res.status(401).json({error:'Authentication required'});try{const payload=verifyAccess(h.slice(7));const u=await db.user.findUnique({where:{id:payload.id},select:{id:true,role:true,companyId:true,status:true,emailVerifiedAt:true,email:true}});if(!u||u.status==='SUSPENDED'||!u.emailVerifiedAt)return res.status(401).json({error:'Session is no longer valid'});req.user={id:u.id,role:u.role,companyId:u.companyId,email:u.email};next();}catch{return res.status(401).json({error:'Invalid or expired session'})}}
