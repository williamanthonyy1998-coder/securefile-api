...
export default function Module() {
  ...
  async function load() {
    try {
      setErr("");
      const needsUsers = ["requests", "approvals", "task-management"].includes(
        name || "",
      );
      const endpoint: any = {
        shared: "/sharing",
      ...
      const jobs: any[] = [];
      if (needsUsers) jobs.push(api("/users"));
      if (endpoint) jobs.push(api(endpoint));
      if (jobs.length) {
        const out = await Promise.all(jobs);
        let i = 0;
        if (needsUsers) setUsers(out[i++] || []);
        if (endpoint) setData(out[i++] || []);
      }
    } catch (e: any) {
      setErr(e.message);
    }
