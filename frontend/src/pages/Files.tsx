...
import { api, directUpload, getPrivatePreviewUrl, downloadPrivateFile } from '../lib/api';
...
export default function Files() {
  ...
  async function openPreview(f:any) {
    ...
    setPreviewZoom(1);
    try {
      const result=await getPrivatePreviewUrl(String(f.id));
      setPreview(result.url);
      setPreviewMime(result.mimeType || f.mimeType || '');
    } catch(e:any){setPreview('');setPreviewMime('');setError(e.message);}
  }

  async function download(f:any) {
    try {
      await downloadPrivateFile(String(f.id), f.name);
    } catch(e:any){setError(e.message);}
  }
        ...
        <table>
          ...
          <tbody>
            ...
            {files.map(f=><tr key={f.id}>
              <td><button className="link-button file-name-button" onClick={()=>openPreview(f)} onDoubleClick={()=>openFilePage(f)} title="Click to preview • Double-click to open">{f.name}</button><small style={{display:'block',color:'#8a96a8'}}>{f.folder?.name||'No folder'}</small></td>
