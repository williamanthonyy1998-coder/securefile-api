...
type Company = {
  id: string; name: string; slug: string; contactEmail: string; logoUrl?: string | null;
  storageLimitGb: number; storageUsedBytes?: string | number;
  subscription?: { status?: string; users?: number; storageGb?: number; months?: number; priceCents?: number; addons?: Record<string, boolean> | null } | null;
...
export default function SuperAdmin() {
  ...
  const [form, setForm] = useState({name:"", email:"", storageGb:"10", users:"1", months:"1", adminName:"", adminEmail:"", adminPassword:"", addons:{preview:false,scanner:false,fax:false,reshare:false,rename:false,postal:false}});
  ...
  const filtered = useMemo(() => {
    ...
    return companies.filter(c => [c.name,c.contactEmail,c.slug].some(v => String(v).toLowerCase().includes(q)));
  }, [companies, query]);
  ...
  return <div className="super-companies">
    ...
    <div className="cards sa-stats">
      ...
      <div className="stat"><span>Status</span><strong>{loading ? "Loading" : "Online"}</strong></div>
    </div>

    <div className="panel">
      <div className="company-toolbar"><div className="company-search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search companies, emails or subdomains..."/></div><span className="muted">{filtered.length} of {companies.length}</span></div>
      ...
      <div className="company-table-wrap"><table className="company-table"><thead><tr><th>Company</th><th>Admin / Contact</th><th>Users</th><th>Storage</th><th>Subscription</th><th>Tenant URL</th><th>Actions</th></tr></thead><tbody>{filtered.map(c=><tr key={c.id}>
        ...
        <td><a href={`${window.location.protocol}//${c.slug}.${window.location.hostname.split(".").slice(-2).join(".")}:5173`} target="_blank" rel="noreferrer">{c.slug}.securefile.com <ExternalLink size={13}/></a></td>
